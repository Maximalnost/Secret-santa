...
const BASE = window.location.pathname.includes("Secret-santa") ? "/Secret-santa" : "";

// Подгружаем данные
async function loadData() {
  participants = await fetch(`${BASE}/data/participants.json`).then(r => r.json());
...
function startMainVideo() {
  ...
  const videoSrc = `${BASE}/${participants[targetId].video}`;
